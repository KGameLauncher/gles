...
interface glTexStorage2D {
    ...
    * _`target`_
    * 
    * Specify the target of the operation. _`target`_ must be one of `GL_TEXTURE_2D`, or `GL_TEXTURE_CUBE_MAP`.
    * 
    * _`levels`_
    * 
    ...
    * 
    * The behavior of `glTexStorage2D` depends on the _`target`_ parameter. When _`target`_ is `GL_TEXTURE_2D`, calling `glTexStorage2D` is equivalent, assuming no errors are generated, to executing the following pseudo-code:
    * 
    *     for (i = 0; i < levels; i++)
    *     {
    ...
    *     }
    * 
    * When _`target`_ is `GL_TEXTURE_CUBE_MAP`, `glTexStorage2D` is equivalent to:
    * 
    *     for (i = 0; i < levels; i++)
    *     {
    ...
    * 
    * `GL_INVALID_VALUE` is generated if _`width`_, _`height`_ or _`levels`_ are less than 1.
    * 
    * `GL_INVALID_OPERATION` is generated if _`levels`_ is greater than log 2 max width ,   height + 1 .
    * 
    * API Version Support
    * -------------------
    * 
    ...
    * --------
    * 
    * [glTexImage2D][de.dasbabypixel.gamelauncher.gles.es20.glTexImage2D.glTexImage2D], [glCompressedTexImage2D][de.dasbabypixel.gamelauncher.gles.es20.glCompressedTexImage2D.glCompressedTexImage2D], [glTexStorage3D][de.dasbabypixel.gamelauncher.gles.es30.glTexStorage3D.glTexStorage3D].
    * 
    * Copyright
    * ---------
    * 
    ...
    * 
    * @see de.dasbabypixel.gamelauncher.gles.es30.glTexStorage2D.glTexStorage2D
    */
    fun glTexStorage2D(target: Int, levels: UInt, internalformat: Int, width: UInt, height: UInt)
}