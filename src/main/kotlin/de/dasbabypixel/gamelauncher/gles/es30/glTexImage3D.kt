...
interface glTexImage3D {
    ...
    * _`target`_
    * 
    * Specifies the target texture. Must be one of `GL_TEXTURE_3D`, `GL_TEXTURE_2D_ARRAY`, or `GL_TEXTURE_CUBE_MAP_ARRAY`.
    * 
    * _`level`_
    * 
    ...
    * 
    * If _`target`_ is `GL_TEXTURE_3D`, data is read from _`data`_ as a sequence of signed or unsigned bytes, shorts, or longs, or single-precision floating-point values, depending on _`type`_. These values are grouped into sets of one, two, three, or four values, depending on _`format`_, to form elements.
    * 
    ...
    * 
    * 
    * If the _`internalFormat`_ parameter is `GL_SRGB`, `GL_SRGB8`, or `GL_SRGB8_ALPHA8`, the texture is treated as if the red, green, blue, or luminance components are encoded in the sRGB color space. Any alpha component is left unchanged. The conversion from the sRGB encoded component c s to a linear component c l is:
    * 
    * c l \= { c s 12.92 if c s ≤ 0.04045 ( c s + 0.055 1.055 ) 2.4 if c s \> 0.04045
    * 
    * Assume c s is the sRGB component in the range \[0,1\].
    * 
    * A one-component texture image uses only the red component of the RGBA color extracted from _`data`_. A two-component image uses the R and A values. A three-component image uses the R, G, and B values. A four-component image uses all of the RGBA components.
    * 
    * Notes
    * -----
    * 
    ...
    * 
    * _`data`_ may be a null pointer. In this case texture memory is allocated to accommodate a texture of width _`width`_, height _`height`_, and depth _`depth`_. You can then download subtextures to initialize this texture memory. The image is undefined if the user tries to apply an uninitialized portion of the texture image to a primitive.
    * 
    * `glTexImage3D` specifies the two-dimensional array, cube map array, or three-dimensional texture for the texture object bound to the current texture unit, specified with [glActiveTexture][de.dasbabypixel.gamelauncher.gles.es20.glActiveTexture.glActiveTexture].
    * 
    * Errors
    * ------
    * 
    ...
    * 
    * `GL_INVALID_ENUM` is generated if _`format`_ is not an accepted format constant. Format constants other than `GL_STENCIL_INDEX` and `GL_DEPTH_COMPONENT` are accepted.
    * 
    * `GL_INVALID_ENUM` is generated if _`type`_ is not a type constant.
    * 
    * `GL_INVALID_VALUE` is generated if _`level`_ is less than 0.
    * 
    * `GL_INVALID_VALUE` may be generated if _`level`_ is greater than log 2 ⁡ max , where _max_ is the returned value of `GL_MAX_3D_TEXTURE_SIZE`.
    * 
    * `GL_INVALID_ENUM` is generated if _`internalFormat`_ is not one of the accepted resolution and format symbolic constants.
    * 
    * `GL_INVALID_VALUE` is generated if _`target`_ is `GL_TEXTURE_CUBE_MAP_ARRAY` and _`width`_ and _`height`_ are not equal, or _`depth`_ is not a multiple of six, indicating 6 \* N layer-faces in the cube map array.
    * 
    * `GL_INVALID_VALUE` is generated if _`width`_, _`height`_, or _`depth`_ is less than 0 or greater than `GL_MAX_3D_TEXTURE_SIZE`.
    * 
    * `GL_INVALID_VALUE` is generated if _`border`_ is not 0 or 1.
    * 
    ...
    * 
    * `GL_INVALID_OPERATION` is generated if _`target`_ is `GL_TEXTURE_3D` and _`format`_ is `GL_DEPTH_COMPONENT`, or `GL_DEPTH_STENCIL`.
    * 
    ...
    */
    fun glTexImage3D(target: Int, level: Int, internalFormat: Int, width: UInt, height: UInt, depth: UInt, border: Int, format: Int, type: Int, data: de.dasbabypixel.gamelauncher.buffers.BufferRO)
}