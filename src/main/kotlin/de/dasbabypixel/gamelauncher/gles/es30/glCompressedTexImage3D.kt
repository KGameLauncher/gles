...
interface glCompressedTexImage3D {
    /**
    * Name
    * ----
    * 
    ...
    * 
    * `glCompressedTexImage3D` loads a previously defined, and retrieved, compressed three-dimensional texture image if _`target`_ is `GL_TEXTURE_3D` (see [glTexImage3D][de.dasbabypixel.gamelauncher.gles.es30.glTexImage3D.glTexImage3D]).
    * 
    * If _`target`_ is `GL_TEXTURE_2D_ARRAY`, _`data`_ is treated as an array of compressed 2D textures.
    * 
    * If _`target`_ is `GL_TEXTURE_CUBE_MAP_ARRAY`, _`data`_ is treated as an array of compressed cube map textures.
    * 
    ...
    * 
    * _`imageSize`_ must be appropriate for the _`width`_, _`height`_ and _`depth`_ of the _`internalformat`_ specified. The size for a single slice of ETC/EAC/ASTC is given in Table 1 below.
    * 
    ...
    */
    fun glCompressedTexImage3D(target: Int, level: Int, internalformat: Int, width: UInt, height: UInt, depth: UInt, border: Int, imageSize: UInt, data: de.dasbabypixel.gamelauncher.buffers.BufferRO)
}