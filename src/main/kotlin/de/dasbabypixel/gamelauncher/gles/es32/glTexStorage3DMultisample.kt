...
interface glTexStorage3DMultisample {
    /**
    * Name
    * ----
    * 
    ...
    * _`target`_
    * 
    * Specifies the target to which the texture object is bound for `glTexStorage3DMultisample`. Must be `GL_TEXTURE_2D_MULTISAMPLE_ARRAY`.
    * 
    * _`samples`_
    * 
    ...
    * -----------
    * 
    * `glTexStorage3DMultisample` specifies the storage requirements for a two-dimensional multisample array texture. Once a texture is specified with this command, its format and dimensions become immutable. The contents of the image may still be modified, however, its storage requirements may not change. Such a texture is referred to as an _immutable-format_ texture.
    * 
    * _`samples`_ specifies the number of samples to be used for the texture and must be greater than zero and less than or equal to the value of `GL_MAX_SAMPLES`. _`internalformat`_ must be a color-renderable, depth-renderable, or stencil-renderable format. _`width`_ and _`height`_ specify the width and height, respectively, of the texture and _`depth`_ specifies the number of layers of the texture. If _`fixedsamplelocations`_ is `GL_TRUE`, the image will use identical sample locations and the same number of samples for all texels in the image, and the sample locations will not depend on the internal format or size of the image.
    * 
    ...
    * 
    * `GL_INVALID_ENUM` is generated if _`target`_ is not `GL_TEXTURE_2D_MULTISAMPLE_ARRAY`.
    * 
    ...
    * 
    * `GL_INVALID_VALUE` is generated if _`depth`_ is less than 1 or greater than the value of `GL_MAX_ARRAY_TEXTURE_LAYERS`.
    * 
    ...
    * ---------------
    * 
    * [glGetInteger](glGetInteger.xhtml) with arguments `GL_MAX_TEXTURE_SIZE`, `GL_MAX_ARRAY_TEXTURE_LEVELS`, `GL_MAX_SAMPLES`, or `GL_TEXTURE_IMMUTABLE_LEVELS`.
    * 
    ...
    * --------
    * 
    * [glTexImage3D][de.dasbabypixel.gamelauncher.gles.es30.glTexImage3D.glTexImage3D], [glTexStorage2DMultisample][de.dasbabypixel.gamelauncher.gles.es31.glTexStorage2DMultisample.glTexStorage2DMultisample], [glTexStorage3D][de.dasbabypixel.gamelauncher.gles.es30.glTexStorage3D.glTexStorage3D].
    * 
    * Copyright
    * ---------
    * 
    ...
    * 
    * @see de.dasbabypixel.gamelauncher.gles.es32.glTexStorage3DMultisample.glTexStorage3DMultisample
    */
    fun glTexStorage3DMultisample(target: Int, samples: UInt, internalformat: Int, width: UInt, height: UInt, depth: UInt, fixedsamplelocations: Boolean)
}