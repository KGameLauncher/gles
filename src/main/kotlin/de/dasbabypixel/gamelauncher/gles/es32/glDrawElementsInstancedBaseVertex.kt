...
interface glDrawElementsInstancedBaseVertex {
    /**
    * Name
    * ----
    * 
    ...
    * -----------
    * 
    * `glDrawElementsInstancedBaseVertex` behaves identically to [glDrawElementsInstanced][de.dasbabypixel.gamelauncher.gles.es30.glDrawElementsInstanced.glDrawElementsInstanced] except that the _i_th element transferred by the corresponding draw call will be taken from element _`indices`_\[i\] + _`basevertex`_ of each enabled array. If the resulting value is larger than the maximum value representable by _`type`_, it is as if the calculation were upconverted to 32-bit unsigned integers (with wrapping on overflow conditions). The operation is undefined if the sum would be negative.
    * 
    * Errors
    * ------
    * 
    ...
    * 
    * `GL_INVALID_VALUE` is generated if _`count`_ or _`primcount`_ is negative.
    * 
    ...
    * 
    * @see de.dasbabypixel.gamelauncher.gles.es32.glDrawElementsInstancedBaseVertex.glDrawElementsInstancedBaseVertex
    */
    fun glDrawElementsInstancedBaseVertex(mode: Int, count: UInt, type: Int, indices: de.dasbabypixel.gamelauncher.buffers.Buffer, primcount: UInt, basevertex: Int)
}