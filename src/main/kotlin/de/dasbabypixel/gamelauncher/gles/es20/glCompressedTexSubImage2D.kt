...
interface glCompressedTexSubImage2D {
    /**
    * Name
    * ----
    * 
    ...
    * 
    * `glCompressedTexSubImage2D` redefines a contiguous subregion of an existing two-dimensional texture image. The texels referenced by _`data`_ replace the portion of the existing texture array with x indices _`xoffset`_ and xoffset + width \- 1 , and the y indices _`yoffset`_ and yoffset + height \- 1 , inclusive. This region may not include any texels outside the range of the texture array as it was originally specified. It is not an error to specify a subtexture with width of 0, but such a specification has no effect.
    * 
    ...
    */
    fun glCompressedTexSubImage2D(target: Int, level: Int, xoffset: Int, yoffset: Int, width: UInt, height: UInt, format: Int, imageSize: UInt, data: Long)
}